<template>
  <div class="module-container">
    <div class="module-header">
      <h2><i class="fas fa-building"></i> Manajemen Client</h2>
      <p class="module-subtitle">Kelola data client (penitip/pengirim barang)</p>
    </div>
    ...
    <div v-if="showForm && canCreate" class="form-container">
      <h3><i class="fas fa-edit"></i> {{ formMode === 'add' ? 'Tambah Client' : 'Edit Client' }}</h3>
      <form @submit.prevent="saveItem">
        <div class="form-group">
          <label><i class="fas fa-user"></i> Nama Client <span class="required">*</span></label>
          <input v-model="form.name" required placeholder="Nama client/perusahaan" />
        </div>
        <div class="form-group">
          <label><i class="fas fa-phone"></i> Telepon</label>
          <input v-model="form.phone" placeholder="0411-123456" />
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label><i class="fas fa-user-tag"></i> PIC</label>
          <input v-model="form.pic_name" placeholder="Person in charge" />
        </div>
    ...
    <p v-else class="empty-message"><i class="fas fa-inbox"></i> Belum ada data client.</p>
  </div>
</template>
...
const exportExcel = async () => {
  try {
    const response = await axios.get('/export/clients', { responseType: 'blob' })
