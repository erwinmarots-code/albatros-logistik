<template>
  <div class="module-container">
      ...
      <table class="modern-table">
        ...
        <tbody>
          <tr v-for="(item, index) in filteredItems" :key="item.id">
            ...
            <td>
              <span :class="'urgency-badge-' + item.urgency">{{ urgencyMap[item.urgency] || item.urgency }}</span>
            </td>
            <td>
            ...
            <td class="action-cell">
              ...
              <template v-if="canCreate && item.status === 'pending'">
                ...
                <button @click="deleteItem(item.id)" class="btn-delete" title="Hapus">
    ...
    <p v-else class="empty-message">
      ...
      {{ searchQuery ? 'Tidak ada pengajuan yang cocok dengan pencarian.' : 'Tidak ada pengajuan yang menunggu persetujuan.' }}
    </p>
  </div>
</template>
...
<style scoped>
/* ====== GAYA KONSISTEN (sama seperti DriverList & HistoryMaintenanceList) ====== */
.module-container {
  max-width: 1200px;
  margin: 0 auto;
}
