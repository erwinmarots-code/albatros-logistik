<template>
  <div class="module-container">
    ...
    <p v-else class="empty-message"><i class="fas fa-inbox"></i> Tidak ada pengajuan biaya yang menunggu persetujuan.</p>
  </div>
</template>
...
const exportExcel = async () => {
  try {
    const response = await axios.get('/export/fuel-expenses', { responseType: 'blob' })
    const url = window.URL.createObjectURL(new Blob([response.data]))
    const link = document.createElement('a')
    link.href = url
    link.setAttribute('download', 'pengajuan_biaya.xlsx')
...
<style scoped>
/* ====== GAYA MODUL SAMA DENGAN VEHICLELIST ====== */
...
/* BADGE */
.type-badge-fuel { background: #ffc107; color: black; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.type-badge-toll { background: #17a2b8; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.type-badge-parking { background: #6f42c1; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.type-badge-meal { background: #fd7e14; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.type-badge-other { background: #6c757d; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.status-badge-pending { background: #ffc107; color: black; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.status-badge-approved { background: #28a745; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }
.status-badge-rejected { background: #dc3545; color: white; padding: 2px 12px; border-radius: 20px; font-size: 12px; font-weight: 600; }

/* TOMBOL AKSI */
.btn-edit, .btn-delete, .btn-approve, .btn-reject {
  border: none; border-radius: 8px; padding: 6px 10px; cursor: pointer; transition: all 0.2s;
