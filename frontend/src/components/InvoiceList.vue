<template>
  <div class="module-container">
    ...
    <div class="toolbar">
      <button v-if="canCreate" @click="openGenerateForm()" class="btn-add"><i class="fas fa-plus-circle"></i> Buat Invoice</button>
      <button @click="fetchItems" class="btn-refresh"><i class="fas fa-sync-alt"></i> Muat Data</button>
    </div>

    <div v-if="showGenerateForm && canCreate" class="form-container">
          ...
          <tbody>
            <tr v-for="(item, index) in items" :key="item.id">
              ...
              <td class="action-cell">
                <button v-if="canViewDetail" @click="viewDetail(item.id)" class="btn-detail" title="Detail"><i class="fas fa-eye"></i></button>
                <button v-if="canUpdateStatus && (item.status === 'draft' || item.status === 'sent')" @click="updateStatus(item.id, 'paid')" class="btn-approve" title="Bayar"><i class="fas fa-check-circle"></i></button>
                <button v-if="canUpdateStatus && (item.status === 'draft' || item.status === 'sent')" @click="updateStatus(item.id, 'cancelled')" class="btn-reject" title="Batal"><i class="fas fa-times-circle"></i></button>
                <button v-if="canDelete" @click="deleteInvoice(item.id)" class="btn-delete" title="Hapus"><i class="fas fa-trash-alt"></i></button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <p v-else class="empty-message"><i class="fas fa-inbox"></i> Belum ada invoice.</p>
    </div>
  </div>
</template>
...
const canCreate = computed(() => ['admin_finance', 'super_admin'].includes(userRole))
const canViewDetail = computed(() => ['admin_finance', 'super_admin'].includes(userRole))
const canUpdateStatus = computed(() => ['admin_finance', 'super_admin'].includes(userRole))
const canDelete = computed(() => ['admin_finance', 'super_admin'].includes(userRole))
...
.btn-add, .btn-refresh { padding: 10px 22px; border: none; border-radius: 10px; cursor: pointer; font-weight: 600; display: inline-flex; align-items: center; gap: 8px; transition: all 0.25s; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
...
.btn-detail, .btn-edit, .btn-delete, .btn-approve, .btn-reject { border: none; border-radius: 8px; padding: 6px 10px; cursor: pointer; transition: all 0.2s; display: inline-flex; align-items: center; justify-content: center; width: 32px; height: 32px; }
