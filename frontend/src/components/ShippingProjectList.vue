<template>
  <div class="module-container">
    ...
    <div v-if="showForm && canCreate" class="form-container">
      ...
      <form @submit.prevent="saveItem">
        ...
        <div class="form-group">
          ...
          <small style="grid-column: 2; font-size: 12px; color: #6c757d;">
    ...
    <p v-else class="empty-message">
      ...
      {{ searchQuery ? 'Tidak ada project yang cocok dengan pencarian.' : 'Belum ada project.' }}
    </p>
  </div>
</template>
...
const filteredItems = computed(() => {
  ...
  return items.value.filter(item =>
    item.resi_number?.toLowerCase().includes(q) ||
    item.no_po?.toLowerCase().includes(q) ||
...
const saveItem = async () => {
  try {
    ...
    const payload = {
      ...
      collie: form.collie === '' ? null : Number(form.collie),
      goods_value: form.goods_value === '' ? null : Number(form.goods_value),
    }

    if (formMode.value === 'add') {
      await axios.post('/shipping-projects', payload)
      alert('Project berhasil dibuat!')
    } else {
      await axios.put(`/shipping-projects/${editingId.value}`, payload)
